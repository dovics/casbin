...
func (rm *RoleManager) geneteTempDomain(domain string) {
...
func (rm *RoleManager) HasLink(name1 string, name2 string, domain ...string) (bool, error) {
	...
	if _, ok := rm.allRoles[domain[0]]; !ok {
		rm.allRoles[domain[0]] = &sync.Map{}
	}
	if rm.hasDomainPattern {
		rm.geneteTempDomain(domain[0])
